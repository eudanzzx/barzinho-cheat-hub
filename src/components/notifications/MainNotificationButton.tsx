...
const MainNotificationButton = () => {
  ...
  useEffect(() => {
    const silencedState = localStorage.getItem('main-notifications-silenced');
  ...
  const toggleNotificationSilence = () => {
    ...
    localStorage.setItem('main-notifications-silenced', newState.toString());
  };
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          className="relative flex items-center gap-1 text-blue-700 bg-blue-100 hover:bg-blue-200 px-2 py-1 sm:px-3 sm:py-2 rounded-xl font-medium shadow border border-blue-200"
        >
      ...
      <DropdownMenuContent align="end" className="w-80 max-h-96 overflow-y-auto bg-white border shadow-lg z-[60]">
        <DropdownMenuLabel className="flex items-center justify-between">
          <span>Notificações Principais</span>
          <Button
            variant="ghost"
            size="sm"
...
export default MainNotificationButton;