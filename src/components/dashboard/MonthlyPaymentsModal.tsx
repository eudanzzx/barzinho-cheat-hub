...
const MonthlyPaymentsModal: React.FC<MonthlyPaymentsModalProps> = ({
  ...
  // Filtrar planos mensais ativos de clientes existentes
  const existingClientNames = new Set(atendimentos.map(a => a.nome));
  
  const activeMonthlyPlanos = planos.filter((plano): plano is PlanoMensal => 
    plano.type === 'plano' && 
    plano.active && 
    !plano.analysisId &&
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-purple-800">
            ...
            Pagamentos Mensais Pendentes
          </DialogTitle>
        </DialogHeader>
        
        <div className="space-y-4">
          {activeMonthlyPlanos.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <CreditCard className="h-12 w-12 mx-auto mb-4 opacity-30" />
              <p>Nenhum pagamento mensal pendente</p>
            </div>
          ) : (
            activeMonthlyPlanos.map((plano) => (
              <Card key={plano.id} className="border-purple-200">
